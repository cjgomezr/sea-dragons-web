...
/**
 * `public.confirmation_email_requests` contra un Postgres desechable. Sostiene
 * el límite del reenvío público de la confirmación (#137), así que lo que se
 * comprueba es que nadie más que el servidor la toca: si la llave anónima
 * pudiera vaciarla, el límite se desactivaría desde el navegador.
 */
  ...
  const rows = await database.query(
    `select privilege_type
      ...
      where table_schema = 'public'
        and table_name = 'confirmation_email_requests'
        and grantee = '${grantee}'
...
describeConPostgres(
  "migración del límite del reenvío de la confirmación",
  () => {
    ...
    it("nace con RLS activo", async () => {
      ...
      const rls = await database.query(
        ...
        where oid = 'public.confirmation_email_requests'::regclass`,
      );
