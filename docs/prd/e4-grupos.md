...
Eso bloquea el plan. Los eventos (E7) y las noticias (E11) tienen que poder dirigirse a un grupo (FR-027), y el alta de miembros por el Admin (E5) asigna grupos desde el primer día (FR-020). Sin grupos, las dos cosas nacen sin audiencia.

## 2. Usuarios y contexto

- **Committee:** la historia del SRD lo dice en sus palabras: quiere organizar a los miembros en grupos como "Senior Squad" o "Junior Squad" para dirigir eventos y noticias con precisión. Lo hace desde el escritorio o el móvil.
...
- Métricas:
  - Crear un grupo y asignarle cinco miembros lleva menos de dos minutos.
  - El conteo de cada grupo coincide siempre con sus miembros asignados que no están dados de baja.
...
- Asignar y quitar miembros de un grupo.
- Una sección **Grupos** en la navegación, solo para Admin, Coach y Committee.
- En Mi cuenta, la lista de grupos a los que pertenece el miembro.
...
- Asignar grupos en el alta de un miembro por el Admin (FR-020): E5.
- Búsqueda, filtros y orden de miembros al asignar: E5.
- Grupos anidados, descripción, color o foto del grupo.
- Avisar al miembro de que lo asignaron o lo quitaron: E6.
- Auditoría de los cambios de grupos: NFR-010 no la pide.

## 5. Requerimientos funcionales

### RF-1 · Los grupos y sus miembros se guardan con reglas en la base · Must
...
- **Dado** un miembro ya asignado a un grupo, **cuando** se lo asigna otra vez, **entonces** sigue habiendo una sola pertenencia.
- **Dado** un grupo con miembros, **cuando** se borra, **entonces** sus pertenencias desaparecen con él y los miembros no se tocan.
- **Dado** un miembro con grupos, **cuando** se borra su identidad, **entonces** sus pertenencias desaparecen.
...
- **Dado** un miembro dado de baja (`inactive`) que sigue asignado, **cuando** se cuenta su grupo, **entonces** no cuenta.
...
- **Dado** el grupo "Senoir Squad", **cuando** se renombra a "Senior Squad", **entonces** conserva sus miembros.
...
- **Dado** un grupo, **cuando** se pide borrarlo, **entonces** la pantalla pide confirmación diciendo cuántos miembros tiene.
- **Dado** esa confirmación, **cuando** se acepta, **entonces** el grupo desaparece de la lista y deja de ofrecerse como audiencia (FR-025, AC-012). Los miembros siguen en el club.

### RF-6 · Asignar y quitar miembros · Must

- **Dado** un miembro sin grupos, **cuando** se lo asigna a "Senior Squad" y a "Masters Squad", **entonces** aparece en la lista de miembros de los dos y los dos conteos suben en uno (FR-026, AC-049).
- **Dado** ese miembro, **cuando** se lo quita de uno, **entonces** ese conteo baja en uno y el otro no cambia.
- **Dado** un miembro dado de baja (`inactive`), **cuando** se intenta asignarlo, **entonces** se rechaza con 422.
- **Dado** un miembro con la cuenta todavía incompleta, **cuando** se lo asigna, **entonces** se permite: el alta por el Admin de E5 crea miembros incompletos que ya pertenecen a sus grupos.
- **Dado** quien asigna, **cuando** busca a quién agregar, **entonces** ve una lista mínima de los miembros del club, solo con nombre, sin correo ni otros datos.
...
- **Dado** un miembro que pertenece a "Senior Squad", **cuando** abre Mi cuenta, **entonces** ve ese grupo en la lista de los suyos.
- **Dado** un miembro sin grupos, **cuando** abre Mi cuenta, **entonces** una frase dice que no pertenece a ninguno.
- **Dado** cualquier miembro, **cuando** abre Mi cuenta, **entonces** no ve los grupos a los que no pertenece ni a los demás miembros de los suyos.
...
- **Dado** un miembro y un conjunto de grupos, **cuando** se pregunta si pertenece a alguno, **entonces** la respuesta sale de las pertenencias guardadas, sin contar a los dados de baja.
...
- **Asignar dos veces al mismo miembro, o doble clic:** una sola pertenencia; la segunda petición responde como si ya estuviera hecho.
- **Quitar a un miembro que ya no estaba:** responde como hecho, sin error.
- **Grupo con muchos miembros:** la lista de miembros del grupo se lee completa; hoy el club tiene decenas, no miles.
...
- **Miembro que pasa a `inactive` después de asignado:** sigue asignado, pero deja de contar y de ser audiencia.
...
- Mockups: no hay pantalla de grupos en `docs/mockups/`. Revisión heurística contra `design-system.md`; la fila de miembro sigue la de `docs/mockups/directory-light.png` (iniciales y nombre).
- Pantallas:
  - **Grupos (Admin, Coach, Committee):** lista de grupos con su conteo, un campo para crear uno, y por grupo las acciones de renombrar y borrar. Al abrir un grupo, sus miembros con la acción de quitar y un selector para agregar miembros.
  - **Mi cuenta (todos):** una sección "Mis grupos" con los nombres de los grupos del miembro.
- Flujo principal: crear el grupo, abrirlo, agregar miembros uno a uno desde el selector, volver a la lista y ver el conteo actualizado.
...
- Privacidad: la lista para asignar miembros devuelve solo id y nombre. Un Player no ve a los demás miembros de sus grupos.
...
- Asignan miembros a grupos Admin, Coach y Committee, los mismos que los crean. Resuelve la contradicción del SRD entre la matriz ("Manage groups" para los tres) y FR-084 ("group assignments editable only by an Admin"): FR-084 y la decisión B3 se leen como "un miembro no puede cambiar sus propios grupos".
...
| --- | ---------------------------------------------------------------------- | ------ | ---------- | ----------------------------------------- |
| 1   | Guarda los grupos y sus miembros en la base, con nombre único por club | S      | ninguna    | No: tabla nueva con datos de miembros     |
| 2   | Deja crear, renombrar, borrar y listar grupos con su conteo, por API   | M      | 1          | No: lógica nueva con permisos             |
| 3   | Deja meter y sacar miembros de un grupo, por API                       | M      | 2          | No: lógica nueva con permisos             |
| 4   | Da a Admin, Coach y Committee una pantalla Grupos                      | M      | 2, 3       | No: pantalla nueva                        |
| 5   | Muestra en Mi cuenta los grupos a los que pertenece el miembro         | S      | 1          | No: cambia una pantalla con datos propios |
